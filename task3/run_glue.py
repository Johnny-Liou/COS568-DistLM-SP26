# coding=utf-8
# Task 3: Distributed training using torch.nn.parallel.DistributedDataParallel (DDP)
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = DistributedSampler(train_dataset, num_replicas=args.world_size, rank=args.local_rank, seed=args.seed)
    ...
    tr_loss = 0.0
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank != 0)
    set_seed(args)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # Log loss for every iteration on every rank
        ...
        # Report timing: discard the first iteration
        if len(iter_times) > 1:
            avg_time = sum(iter_times[1:]) / len(iter_times[1:])
            print(f"[Rank {args.local_rank}] Avg time per iter (excluding first): {avg_time:.4f}s over {len(iter_times)-1} iters")
...
def main():
    ...
    logger.warning("Rank: %s, device: %s, world_size: %s", args.local_rank, args.device, args.world_size)
    ...
    # Only rank 0 downloads the model; others wait
    if args.local_rank != 0:
        torch.distributed.barrier()
    ...
    # Only rank 0 runs evaluation; unwrap the DDP module first
    if args.local_rank == 0:
        evaluate(args, model.module, tokenizer, prefix="")
